...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        // -------------------------------------------------------------
        // REBIRTH PATH
        // If player is a Magician (200) and has Rebirths, we allow re-entry
        // regardless of previous quest completion status.
        // -------------------------------------------------------------
        if (cm.getPlayer().getReborns() > 0 && cm.getJob() == 200) {
            if (status == 0) {
                 cm.sendNext("I see the wisdom of a veteran magician in you. Although you have been reborn, you must prove your skills again for this life.");
            } else if (status == 1) {
                 cm.sendAcceptDecline("I will let you enter the testing grounds immediately. Are you ready?");
            } else if (status == 2) {
                 // Warp directly to the Magician Test Map (108000200)
                 cm.warp(108000200, 0);
                 cm.dispose();
            }
            return;
        ...
        if (status == 0) {
            ...
            // Only run quest update logic if strictly necessary
            if (!cm.isQuestCompleted(100006)) {
                cm.completeQuest(100006);
                cm.startQuest(100007);
                cm.gainItem(4031009, -1); // Remove Letter from Grendel
            }
        } else if (status == 5) {
            cm.warp(108000200, 0);
            cm.dispose();
