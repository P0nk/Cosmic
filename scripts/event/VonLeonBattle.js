...
function clearPQ(eim) {
    eim.stopEventTimer();
    eim.setEventCleared(ExpeditionType.VONLEON);
}
...
function monsterKilled(mob, eim) {
    ...
    if (isVonleon(mob)) {
        ...
        for (var i = 0; i < party.size(); i++) {
            party.get(i).getClient().getAbstractPlayerInteraction().gainItem(4001126, 60);
            party.get(i).gainExp(884000, true, true, true); // Grant 884,000 exp to each player
        }
