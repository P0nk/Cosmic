...
var mobsToSpawn = [9990150]; // Mob IDs to spawn

function setup(level, lobbyid) {
    var eim = em.newInstance("Normal Lucid" + lobbyid);
    eim.setProperty("level", level);
    eim.setProperty("boss", "0");
    eim.setIntProperty("fallenPlayers", 0);

    eim.getInstanceMap(450004751).resetPQ(level);
...
function spawnMob1(eim) {
    ...
    var spawnX = 1058; // Example position, adjust as needed
    var spawnY = 48;
...
function clearPQ(eim) {
    eim.stopEventTimer();
    eim.setEventCleared(ExpeditionType.NLUCID);
}

function isLucid(mob) {
    var mobid = mob.getId();
    return mobid == 9990150;
}
...
function monsterKilled(mob, eim) {
    if (isLucid(mob)) {
      ...
      //  mob.getMap().broadcastLucidVictory();
    }
}
