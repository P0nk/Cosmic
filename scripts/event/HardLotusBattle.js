...
function setup(level, lobbyid) {
    var eim = em.newInstance("Hard Lotus" + lobbyid);
    eim.setProperty("level", level);
    eim.setProperty("boss", "0");
    eim.setIntProperty("defeatedBoss", 0);
    eim.setIntProperty("fallenPlayers", 0);

    eim.getInstanceMap(1065).resetPQ(level);
...
function playerRevive(eim, player) {
    player.death// player presses ok on the death pop up.
    if (eim.isExpeditionTeamLackingNow(true, minPlayers, player)) {
        eim.unregisterPlayer(player);
        end(eim);
    } else {
...
function clearPQ(eim) {
    eim.stopEventTimer();
    eim.setEventCleared(ExpeditionType.HARDLOTUS);
}

function isLotus(mob) {
    var mobid = mob.getId();
    return mobid == 8240099;
}
