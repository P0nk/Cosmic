...
var mobsToSpawn = [9990301]; // Mob IDs to spawn

function setup(level, lobbyid) {
    var eim = em.newInstance("Normal Will" + lobbyid);
    eim.setProperty("level", level);
    eim.setProperty("boss", "0");
    eim.setIntProperty("fallenPlayers", 0);

    eim.getInstanceMap(450007441).resetPQ(level);
...
function spawnMob1(eim) {
    ...
    var spawnX = -1161; // Example position, adjust as needed
    var spawnY = 215;
...
function clearPQ(eim) {
    eim.stopEventTimer();
    eim.setEventCleared(ExpeditionType.NWILLSPIDER);
}

function isWill(mob) {
    var mobid = mob.getId();
    return mobid == 9990301;
}
