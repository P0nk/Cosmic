...
public final class FoodDropper {
    ...
    // 5000 = 0.5%
    private static final int CHANCE_PRIMARY = 7500;       // The main tier for the mob's level
    private static final int CHANCE_SECONDARY = 3500;     // +/- 1 Tier (e.g. T7 & T9 for a T8 mob)
    private static final int CHANCE_TERTIARY = 1500;       // +/- 2 Tiers (e.g. T6 & T10 for a T8 mob)
    ...
    public static void dropForMonster(
        ...
        // 3. Iterate through offsets: -2 (Tertiary), -1 (Secondary), 0 (Primary), +1 (Secondary), +2 (Tertiary)
        // This ensures separate independent rolls for each relevant tier.
        for (int offset = -2; offset <= 2; offset++) {

            int targetOrdinal = primaryOrdinal + offset;

            // Bounds check: ensure we don't look for Tiers lower than T1 or higher than LEGENDARY
            ...
            // Determine Base Chance based on distance from primary tier
            int baseChance;
            int absOffset = Math.abs(offset);
            if (absOffset == 0) {
                baseChance = CHANCE_PRIMARY;      // The exact tier for this level
            } else if (absOffset == 1) {
                baseChance = CHANCE_SECONDARY;    // +/- 1 tier
            } else {
                baseChance = CHANCE_TERTIARY;     // +/- 2 tiers
            }

            // Calculate final probability threshold
            // Logic: Threshold = Base * BossMulti * ServerRate
            // Example: 5000 * 20 (Boss) * 1 (Server) = 100,000 (10% chance)
            long finalThreshold = baseChance * bossMulti * serverRate;

            // Perform rolls (usually 1, but loops if 'rolls' arg > 1)
            int attempts = Math.max(1, rolls);
            for (int i = 0; i < attempts; i++) {
                // Roll 0 to 999,999
                if (RNG.nextInt(1_000_000) < finalThreshold) {
                    // Success! Pick a random item from this specific tier
